...
Namespace Common.Utilities
    Public Class SqlDB
        ...
        Public Const ConfigFlieName As String = "C:\Windows\DLT-Config.ini"
        Private Const DbSectionName As String = "DatabaseConfig"
        Private Const VarDataSource As String = "DataSource"
        Private Const VarDbName As String = "DbName"
        Private Const VarDbUserId As String = "DbUserID"
        Private Const VarDbPassword As String = "DbPassword"

        Private Shared Function GetDbConfig(ByVal section As String, ByVal key As String) As String
            Dim ret As String = ""
            Dim ini As New Linq.Utilities.IniReader(ConfigFlieName)
            ini.Section = section

            Dim iniSection As ArrayList = ini.GetSectionNames()
            Dim sec = From se In iniSection Where se = section Select se

            If sec.Count() = 0 Then
                ini.Write(VarDataSource, "Akkarawat")
                ini.Write(VarDbName, "DLT-SCAN")
                ini.Write(VarDbUserId, "sa")
                ini.Write(VarDbPassword, "kugsu01#")
            End If

            ret = ini.ReadString(key)
            Return ret
        End Function

        Public Shared ReadOnly Property DataSource() As String
            Get
                Return GetDbConfig(DbSectionName, VarDataSource) '"sourcesafe" 'ConfigurationSettings.AppSettings("ServerName").ToString ' "Akkarawat"
            End Get
        End Property
        Public Shared ReadOnly Property DbName()
            Get
                Return GetDbConfig(DbSectionName, VarDbName) '"DLT-SCAN" 'ConfigurationSettings.AppSettings("DbName").ToString
            End Get
        End Property
        Public Shared ReadOnly Property DbUserID() As String
            Get
                Return GetDbConfig(DbSectionName, VarDbUserId) '"sa" 'ConfigurationSettings.AppSettings("DbUserID").ToString
            End Get
        End Property
        Public Shared ReadOnly Property DbPwd() As String
            Get
                Return GetDbConfig(DbSectionName, VarDbPassword) ' "1qaz@WSX" 'ConfigurationSettings.AppSettings("DbPwd").ToString
            End Get
        End Property
